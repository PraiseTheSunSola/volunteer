document.addEventListener("DOMContentLoaded", function() {
    ...
    const startDateInput = document.getElementById("participation-start");
    const endDateInput = document.getElementById("participation-end");
    ...
    submitButton.addEventListener("click", function(event) {
        if (!validateDates()) {
            ...
            formData.append("participationStart", startDateISO);
            formData.append("participationEnd", endDateISO);

            // 폼 제출 처리 (서버로 데이터를 보낼 수 있음)
            fetch("/participation/participationWriteSave", {
                method: "POST",
                body: formData
